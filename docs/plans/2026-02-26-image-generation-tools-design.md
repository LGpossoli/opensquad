# Design: Image Generation Tools for SquadOS
...
SquadOS squads currently produce text-only output. The instagram-content squad generates carousel copy in markdown but cannot produce actual images. This design adds two new tools to the SquadOS registry that enable squads to generate production-ready social media images.
