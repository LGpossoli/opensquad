...
**Architecture:** Sherlock is a subagent the Architect dispatches during a new Phase 1.5 (Investigation Setup) of the create-squad workflow. One Sherlock subagent runs per profile URL in parallel. Each uses Playwright MCP for navigation and yt-dlp + Whisper for video transcription. Output is saved to `_squados/_investigations/{squad-code}/` as raw-content.md + pattern-analysis.md per profile, plus a consolidated-analysis.md across all profiles.
...
**Files:**
- Create: `templates/_squados/core/prompts/sherlock.prompt.md`
...
The profile is stored at `_squados/_browser_profile/`.
...
Save to: `_squados/_investigations/{squad-code}/{platform}-{username}/raw-content.md`
...
Save to: `_squados/_investigations/{squad-code}/{platform}-{username}/pattern-analysis.md`
...
Save to: `_squados/_investigations/{squad-code}/consolidated-analysis.md`
...
Run: `wc -l templates/_squados/core/prompts/sherlock.prompt.md`
Expected: 300+ lines

**Step 3: Commit**

```bash
git add templates/_squados/core/prompts/sherlock.prompt.md
...
The `_squados/_investigations/` directory needs to exist when a user initializes SquadOS.

**Files:**
- Modify: `templates/_squados/_investigations/.gitkeep` (create as template)
...
Expected: FAIL — `_squados/_investigations` directory does not exist

**Step 3: Create the template directory marker**

Create file: `templates/_squados/_investigations/.gitkeep`
Content: empty file
...
```bash
git add templates/_squados/_investigations/.gitkeep tests/init.test.js
...
**Files:**
- Modify: `templates/_squados/core/architect.agent.yaml` (lines 62-131 are the create-squad workflow)
...
```yaml
      ...
      4. **Read Sherlock instructions**:
         Read `_squados/core/prompts/sherlock.prompt.md` for the full investigation guide.

      5. **Create investigation directory**:
         Create `_squados/_investigations/{squad-code}/` via mkdir.
...
```yaml
      ...
      Read `_squados/_investigations/{squad-code}/consolidated-analysis.md` and
      ...
      investigation:
        ...
        profiles_analyzed: 3
        date: 2026-02-24
        dir: _squados/_investigations/{squad-code}
      ```
```
...
```markdown
- `_squados/_investigations/` — Sherlock content investigations (profile analyses)
```

After the `_squados/_memory/` line.
...
Add investigation reference to the SquadOS skill help.

**Files:**
- Modify: `templates/.claude/skills/squados/SKILL.md`
...
```markdown
| `/squados create <desc>` with URLs | Load Architect → Create Squad with Sherlock investigation |
```

In the help text section, update the EXAMPLES:

```
  /squados create "Instagram carousel squad" (+ provide reference profile URLs when asked)
```

**Step 2: Commit**

```bash
git add templates/.claude/skills/squados/SKILL.md
...
Read `templates/_squados/core/prompts/sherlock.prompt.md` and verify:
...
Read `templates/_squados/core/architect.agent.yaml` and verify:
...
- `_squados/core/prompts/sherlock.prompt.md` — exists
- `_squados/_investigations/` — created by init
...
|--------|------|---------------------|
| Create | `templates/_squados/core/prompts/sherlock.prompt.md` | ~500 |
| Create | `templates/_squados/_investigations/.gitkeep` | 0 (empty) |
| Modify | `templates/_squados/core/architect.agent.yaml` | +80-100 |
| Modify | `templates/CLAUDE.md` | +3 |
| Modify | `templates/.claude/skills/squados/SKILL.md` | +3 |
