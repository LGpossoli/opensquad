...
Replace the Figma MCP tool in the SquadOS registry with Canva Connect MCP. The Architect auto-suggests Canva when creating squads that need design/visual content. No structural changes to the tools engine.
...
Remove entirely from both `_squados/tools/registry/` and `templates/_squados/tools/registry/`.
