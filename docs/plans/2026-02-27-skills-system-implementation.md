...
**Goal:** Unify "tools" and "skills" into a single "skills" concept across the entire SquadOS codebase.

**Architecture:** Replace `_squados/tools/` with `_squados/skills/`, convert all `.tool.yaml` files to `SKILL.md` format, rewrite the engine/runner/architect to use skills terminology, and update CLI to install skills to the new location.
...
- `_squados/tools/registry/apify.tool.yaml`
- `_squados/tools/registry/canva.tool.yaml`
- `_squados/tools/registry/instagram-publisher.tool.yaml`
- `_squados/tools/registry/asset-fetcher.tool.yaml`
- `_squados/tools/registry/blotato.tool.yaml`
- `_squados/tools/registry/visual-renderer.tool.yaml`
...
```markdown
# SquadOS Skills Catalog

Available skills for SquadOS squads. Install via CLI or chat.

## Installation

\`\`\`bash
npx squados install <skill-name>
\`\`\`

Or in your IDE chat:
\`\`\`
/squados install <skill-name>
\`\`\`
...
## Task 2: Create the squados-skill-creator

Create the skill that helps users create new SquadOS skills. Adapted from Anthropic's skill-creator (`temp/skill-creator/`).

**Files:**
- Create: `skills/squados-skill-creator/SKILL.md`
- Create: `skills/squados-skill-creator/agents/grader.md`
- Create: `skills/squados-skill-creator/agents/comparator.md`
- Create: `skills/squados-skill-creator/agents/analyzer.md`
- Create: `skills/squados-skill-creator/references/schemas.md`
- Create: `skills/squados-skill-creator/references/skill-format.md`
- Create: `skills/squados-skill-creator/scripts/aggregate_benchmark.py`
- Create: `skills/squados-skill-creator/scripts/generate_report.py`
- Create: `skills/squados-skill-creator/scripts/run_eval.py`
...
1. **Frontmatter**: `name: squados-skill-creator`, description focused on creating SquadOS skills
...
4. **Output location**: Skills go to `_squados/skills/<name>/` (not `.claude/skills/`)
...
- Create: `skills/squados-skill-creator/eval-viewer/generate_review.py`
- Create: `skills/squados-skill-creator/eval-viewer/viewer.html`
- Create: `skills/squados-skill-creator/assets/eval_review.html` (for eval set review UI)

**Step 6: Commit**

```bash
git add skills/squados-skill-creator/
git commit -m "feat: create squados-skill-creator skill"
```
...
**Files:**
- Create: `_squados/core/skills.engine.md`
- Remove: `_squados/core/tools.engine.md` (done in Task 12)

**Step 1: Write skills.engine.md**

Create `_squados/core/skills.engine.md` based on the design doc's "Skills Engine" section. This replaces the 245-line `tools.engine.md`.

Key differences from `tools.engine.md`:
- **No registry**: Skills are installed directly as folders in `_squados/skills/`
...
```markdown
# SquadOS Skills Engine

You are the Skills Engine. Your job is to manage skill integrations for SquadOS squads.
...
## File Locations
- **Installed skills**: `_squados/skills/` — each skill in its own subfolder
- **Skill catalog**: https://github.com/squados-ai/squados/tree/main/skills

## SKILL.md Format
[Reference to skill-format.md in squados-skill-creator]

## Operations

### 1. List Installed Skills
1. Read all subdirectories in `_squados/skills/`
...
2. Fetch skill folder contents from GitHub:
   - Fetch `https://raw.githubusercontent.com/squados-ai/squados/main/skills/<name>/SKILL.md`
   - For skills with scripts/references/assets: fetch each listed file
3. Write to `_squados/skills/<name>/`
...
3. For each non-native skill:
   a. Check `_squados/skills/<name>/` exists
...
1. Fetch skills/README.md from GitHub (or list `_squados/skills/` for already-installed)
...
```bash
git add _squados/core/skills.engine.md
...
The current `src/skills.js` installs skills to `.claude/skills/`. Rewrite it to install to `_squados/skills/`.
...
- `listInstalled` should scan `_squados/skills/` (not `.claude/skills/`)
- `listInstalled` should exclude `squados-skill-creator` (the built-in, like old code excluded `squados`)
- `installSkill` should write to `_squados/skills/<id>/SKILL.md`
- `installSkill` should handle skills with subdirectories (scripts/, references/, etc.)
- `removeSkill` should delete from `_squados/skills/<id>/`
...
const REGISTRY_BASE = 'https://raw.githubusercontent.com/squados-ai/squados/main';

export async function listInstalled(targetDir) {
  try {
    const skillsDir = join(targetDir, '_squados', 'skills');
    const entries = await readdir(skillsDir, { withFileTypes: true });
    return entries
      .filter((e) => e.isDirectory() && e.name !== 'squados-skill-creator')
      .map((e) => e.name);
  } catch (err) {
...
export async function installSkill(id, targetDir, fetcher = fetch) {
  ...
  const destDir = join(targetDir, '_squados', 'skills', id);
...
export async function removeSkill(id, targetDir) {
  validateSkillId(id);
  const skillDir = join(targetDir, '_squados', 'skills', id);
...
export async function getSkillVersion(id, targetDir) {
  try {
    const skillMd = await readFile(
      join(targetDir, '_squados', 'skills', id, 'SKILL.md'), 'utf-8'
    );
...
git commit -m "feat: rewrite skills.js to install to _squados/skills/"
```

---

## Task 5: Update CLI (bin/squados.js + src/skills-cli.js)

Update CLI to support new command structure: `npx squados install/uninstall/update/skills`.

**Files:**
- Modify: `bin/squados.js:1-40`
- Modify: `src/skills-cli.js:1-143`

**Step 1: Update bin/squados.js**
...
if (command === 'init') {
  ...
  // 'npx squados update' with no args = update core
  ...
  console.log(`
  squados — Multi-agent orchestration for Claude Code

  Usage:
    npx squados init                    Initialize SquadOS in current directory
    npx squados update                  Update SquadOS core to the latest version
    npx squados install <name>          Install a skill
    npx squados uninstall <name>        Remove a skill
    npx squados update <name>           Update a specific skill
    npx squados skills                  List installed skills

  Learn more: https://github.com/squados-ai/squados
  `);
...
Update to use the new `listInstalled` from `src/skills.js` (which now reads `_squados/skills/`).

Key changes:
- `runList`: Instead of fetching manifest, just list installed skills from `_squados/skills/` and show a link to the GitHub catalog
- `runInstall`: Install to `_squados/skills/` (already handled by updated `skills.js`)
- `runRemove`: Remove from `_squados/skills/`
...
```bash
git add bin/squados.js src/skills-cli.js
...
Update the init flow to copy `squados-skill-creator` instead of the tools directory.
...
The `copyCommonTemplates` function copies everything from `templates/` except `ide-templates/`. Since we'll update the templates directory (Task 10) to have `_squados/skills/` instead of `_squados/tools/`, the init.js code should work without changes.

Verify by reading the function — it recursively copies all files from `templates/` preserving directory structure. If `templates/_squados/skills/squados-skill-creator/` exists and `templates/_squados/tools/` is removed, init will automatically do the right thing.
...
**Step 1: Add _squados/skills to protected paths**

User-installed skills should NOT be overwritten during update. Only the `squados-skill-creator` (which comes from templates) should be updated.

```javascript
const PROTECTED_PATHS = [
  '_squados/_memory',
  '_squados/_investigations',
  '_squados/skills',     // Don't overwrite user-installed skills
  'squads',
];
```
...
Actually, the template only contains `squados-skill-creator`. User-installed skills are NOT in templates. So the update would only copy the skill-creator, which is correct — we WANT to update the skill-creator.

But with `_squados/skills` in PROTECTED_PATHS, we'd block that update. So we should NOT add it to protected paths. The update will:
1. Copy `templates/_squados/skills/squados-skill-creator/` → overwrites with latest version ✅
2. User-installed skills (e.g., `_squados/skills/canva/`) are NOT in templates, so they won't be touched ✅
...
**Files:**
- Modify: `_squados/core/architect.agent.yaml`
...
3. Line 42: `"Tool discovery: after extraction, scan _squados/tools/registry/..."` → `"Skill discovery: after extraction, scan _squados/skills/ for installed skills and check the GitHub catalog for additional skills matching the squad's needs"`
4. Lines 303-322: Rewrite Phase 3.5 "Tool Discovery" → "Skill Discovery":
   - Read installed skills from `_squados/skills/`
...
```bash
git add _squados/core/architect.agent.yaml
...
**Files:**
- Modify: `_squados/core/runner.pipeline.md:16-24, 85-95`
...
1. Lines 16-24 (Initialization step 3 "Resolve tools"):
   ...
   3. **Resolve skills**: Read `squad.yaml` → `skills` section. For each non-native skill:
      a. Verify `_squados/skills/{skill}/SKILL.md` exists
         - If missing → prompt user: "Skill '{skill}' is not installed. Install now? (y/n)"
         - If yes → read `_squados/core/skills.engine.md`, follow Operation 2 (Install)
         - If no → **ERROR**: stop pipeline
...
2. Lines 85-95 (Agent loading step 4 "Inject tool instructions"):
   ...
   4. **Inject skill instructions**: Check which skills the agent declares in its frontmatter `skills:`.
      ...
      a. Read `_squados/skills/{skill}/SKILL.md`
...
```bash
git add _squados/core/runner.pipeline.md
...
**Files:**
- Remove: `templates/_squados/tools/` (entire directory)
- Remove: `templates/_squados/core/tools.engine.md`
- Remove: `templates/squads/instagram-content/tools/` (directory)
- Create: `templates/_squados/skills/squados-skill-creator/` (copy from `skills/squados-skill-creator/`)
- Create: `templates/_squados/core/skills.engine.md` (copy from `_squados/core/skills.engine.md`)
- Modify: `templates/_squados/core/architect.agent.yaml` (copy updated version)
- Modify: `templates/_squados/core/runner.pipeline.md` (copy updated version)
...
```bash
rm -rf templates/_squados/tools/
rm -f templates/_squados/core/tools.engine.md
...
cp -r skills/squados-skill-creator/ templates/_squados/skills/squados-skill-creator/

# Copy updated core files to templates
cp _squados/core/skills.engine.md templates/_squados/core/skills.engine.md
cp _squados/core/architect.agent.yaml templates/_squados/core/architect.agent.yaml
cp _squados/core/runner.pipeline.md templates/_squados/core/runner.pipeline.md
```
...
Update all IDE-specific template files and the live `/squados` skill.

**Files:**
- Modify: `.claude/skills/squados/SKILL.md` (live — current project)
- Modify: `templates/ide-templates/claude-code/.claude/skills/squados/SKILL.md`
...
**Step 1: Update .claude/skills/squados/SKILL.md (live)**

This is the main `/squados` skill with 200 lines. Changes:
...
2. **Command Routing** (lines 57-73): Replace tools routes with skills:
   ...
   ```
   Remove: `| /squados tools |`

3. **Help Text** (lines 77-121): Remove TOOLS section, update SKILLS section:
   ```
   SKILLS
     /squados skills           Browse installed skills
     /squados install <name>   Install a skill from catalog
     /squados uninstall <name> Remove a skill
   ```
...
4. **Loading the Tools Engine** (lines 153-164): Replace entirely with "Loading the Skills Engine":
   ...
   When the user selects "Skills" or types `/squados skills`:

   1. Read `_squados/core/skills.engine.md` for the skills engine instructions
   2. Present the skills submenu using AskUserQuestion:
      ...
      - **Create a skill** — Create a custom skill (uses squados-skill-creator)
...
Apply the same changes to `templates/ide-templates/claude-code/.claude/skills/squados/SKILL.md`.
...
- Line 61: `/squados tools` → `/squados skills`
...
```bash
git add .claude/skills/squados/SKILL.md templates/ide-templates/
...
```
- `_squados/skills/` — Installed skills (integrations, scripts, prompts)
```
...
```markdown
| `/squados skills` | Browse and install skills |
| `/squados install <name>` | Install a skill |
```
...
- Update `skillsInstallHint` to use new command format: `Install with: npx squados install <id>`
...
Ensure all tests use `_squados/skills/` paths instead of `.claude/skills/`:
- `listInstalled` scans `_squados/skills/`, excludes `squados-skill-creator`
- `installSkill` writes to `_squados/skills/<id>/SKILL.md`
- `removeSkill` deletes from `_squados/skills/<id>/`
- Remove `fetchManifest` tests (function removed)
- Add `getSkillVersion` tests

**Step 2: Update tests/init.test.js**

If tests assert on created file paths, update to expect `_squados/skills/squados-skill-creator/` instead of `_squados/tools/`.
...
**Files:**
- Remove: `_squados/tools/` (entire directory)
- Remove: `_squados/core/tools.engine.md`

**Step 1: Remove old tools directory**

```bash
rm -rf _squados/tools/
rm -f _squados/core/tools.engine.md
```

**Step 2: Copy skills engine to working directory**

The `skills.engine.md` was created in Task 3 for the working directory.
Verify it exists at `_squados/core/skills.engine.md`.

**Step 3: Copy skill-creator to working directory**

```bash
cp -r skills/squados-skill-creator/ _squados/skills/squados-skill-creator/
```

**Step 4: Verify directory structure**

```bash
# Should show skills/ with squados-skill-creator
ls _squados/skills/

# Should NOT exist
ls _squados/tools/ 2>/dev/null && echo "ERROR: tools still exists" || echo "OK: tools removed"

# Should show skills.engine.md, NOT tools.engine.md
ls _squados/core/*.engine.md
```
...
**Files:**
- Check: `_squados/core/prompts/*.prompt.md`
- Check: `templates/_squados/core/prompts/*.prompt.md`

**Step 1: Search for "tool" references in prompts**

```bash
grep -r "tool" _squados/core/prompts/ --include="*.md" -l
```

**Step 2: Update any found references**

For each file with "tool" references:
- Replace "tool" → "skill" where it refers to the SquadOS concept
...
```bash
cp _squados/core/prompts/*.prompt.md templates/_squados/core/prompts/
```

**Step 4: Commit**

```bash
git add _squados/core/prompts/ templates/_squados/core/prompts/
