...
**Files:**
- Create: `_squados/core/formats/instagram-feed.md`

**Step 1: Create the directory**

```bash
mkdir -p "_squados/core/formats"
```

**Step 2: Create instagram-feed.md**

Migrate content from `_squados/core/platforms/instagram.md` into the format schema. The format file must include YAML frontmatter with parseable constraints, plus 6 markdown sections. Use the existing instagram.md as the source — restructure it into the format schema:
...
```bash
git add _squados/core/formats/instagram-feed.md
...
**Files:**
- Create: `_squados/core/formats/instagram-reels.md`
- Create: `_squados/core/formats/instagram-stories.md`

**Step 1: Create instagram-reels.md**

Source: Reels-specific content from `_squados/core/platforms/instagram.md`.
...
```bash
git add _squados/core/formats/instagram-reels.md _squados/core/formats/instagram-stories.md
...
**Files:**
- Create: `_squados/core/formats/linkedin-post.md`
- Create: `_squados/core/formats/linkedin-article.md`

**Step 1: Create linkedin-post.md**

Source: `_squados/core/platforms/linkedin.md` — Text Posts and Document/Carousel sections.
...
```bash
git add _squados/core/formats/linkedin-post.md _squados/core/formats/linkedin-article.md
...
**Files:**
- Create: `_squados/core/formats/twitter-post.md`
- Create: `_squados/core/formats/twitter-thread.md`

**Step 1: Create twitter-post.md**

Source: `_squados/core/platforms/twitter.md` — Standalone Tweets section.
...
```bash
git add _squados/core/formats/twitter-post.md _squados/core/formats/twitter-thread.md
...
**Files:**
- Create: `_squados/core/formats/youtube-script.md`
- Create: `_squados/core/formats/youtube-shorts.md`

**Step 1: Create youtube-script.md**

Source: `_squados/core/platforms/youtube.md` — Long-form Video section.
...
```bash
git add _squados/core/formats/youtube-script.md _squados/core/formats/youtube-shorts.md
...
**Files:**
- Create: `_squados/core/formats/whatsapp-broadcast.md`
- Create: `_squados/core/formats/email-newsletter.md`
- Create: `_squados/core/formats/email-sales.md`
- Create: `_squados/core/formats/blog-post.md`
- Create: `_squados/core/formats/blog-seo.md`
...
```bash
git add _squados/core/formats/whatsapp-broadcast.md _squados/core/formats/email-newsletter.md _squados/core/formats/email-sales.md _squados/core/formats/blog-post.md _squados/core/formats/blog-seo.md
...
**Files:**
- Modify: `_squados/core/runner.pipeline.md` (lines 74-111, Agent Loading section)

**Step 1: Add format injection logic**

In `_squados/core/runner.pipeline.md`, after the overlay resolution section (line 94) and before the skill injection section (line 100), add a new subsection for format injection.
...
4. **Inject format context**: Check if the current step's frontmatter contains a `format:` field.
   If present:
   a. Read `_squados/core/formats/{format}.md` (e.g., `_squados/core/formats/instagram-feed.md`)
      - If the file does not exist → **WARNING**: "Format '{format}' not found in _squados/core/formats/. Skipping format injection." Continue without format.
...
**Files:**
- Modify: `_squados/core/architect.agent.yaml` (Phase 1: Discovery section, lines 65-118)
...
```yaml
      7. **Target Format** (for content squads only): After understanding the purpose,
         ...
         List available formats by scanning `_squados/core/formats/` directory.
...
**Files:**
- Modify: `_squados/core/architect.agent.yaml` (Phase 4: Design, Phase 5: Build)
...
Add a comment explaining: "The `format:` field is optional. When present, the Pipeline Runner automatically injects the format file from `_squados/core/formats/{format}.md` into the agent's context. Use this for any step where platform-specific content rules should guide the agent."
...
Replace the current reference to `_squados/core/platforms/{platform}.md`:
```
- Read `_squados/core/platforms/{platform}.md` for platform knowledge
```
...
- Use the format system: assign `format: {format-id}` to each creator step
  ...
  The format file replaces the old `_squados/core/platforms/` files.
```

Also update the line at ~924 that says:
```
- Also load platform knowledge: `_squados/core/platforms/{platform}.md`
```
...
**Files:**
- Modify: `_squados/core/platforms/instagram.md`
- Modify: `_squados/core/platforms/linkedin.md`
- Modify: `_squados/core/platforms/twitter.md`
- Modify: `_squados/core/platforms/youtube.md`
...
```markdown
> ⚠️ **DEPRECATED**: This file is superseded by format files in `_squados/core/formats/`.
...
```bash
git add _squados/core/platforms/
...
Read `_squados/core/runner.pipeline.md` and confirm:
...
Search `_squados/core/architect.agent.yaml` for:
- No remaining references to `_squados/core/platforms/` (should all be replaced)
