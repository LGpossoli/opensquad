...
**Goal:** During `squados init`, ask which IDEs the user wants to install (multi-select) and copy the appropriate config files for each selected IDE (Claude Code, Open Code, Codex, Antigravity).
...
```bash
mkdir -p "templates/ide-templates/claude-code/.claude/skills/squados"
cp "templates/.claude/skills/squados/SKILL.md" "templates/ide-templates/claude-code/.claude/skills/squados/SKILL.md"
...
Expected: All tests PASS. The test `init creates _squados directory structure` checks for `.claude/skills/squados/SKILL.md` — this now comes from `copyIdeTemplates(['claude-code'])` which is the default.
...
When the user types `/squados` or asks for the menu, present an interactive selector using AskUserQuestion with these options (max 4 per question):
...
| `/squados dashboard <name>` | Load dashboard.prompt.md → generate squad dashboard |
| `/squados edit <name> <changes>` | Load Architect → Edit Squad flow |
| `/squados tools` | Load Tools Engine → Show tools menu |
...
```bash
node bin/squados.js init /tmp/squados-smoke-test
```
...
**Files:**
- Verify: `templates/ide-templates/claude-code/.claude/skills/squados/SKILL.md`

**Step 1: Check if the template SKILL.md matches the live SKILL.md**

```bash
diff templates/ide-templates/claude-code/.claude/skills/squados/SKILL.md .claude/skills/squados/SKILL.md
```

Expected: If there are differences, the live `.claude/skills/squados/SKILL.md` is the source of truth. Copy it over:

```bash
cp .claude/skills/squados/SKILL.md templates/ide-templates/claude-code/.claude/skills/squados/SKILL.md
```
