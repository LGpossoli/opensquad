...
4. **Inject format context**: Check if the current step's frontmatter contains a `format:` field.
   If present:
   a. Read `_squados/core/best-practices/{format}.md` (e.g., `_squados/core/best-practices/instagram-feed.md`)
      - If the file does not exist → **WARNING**: "Format '{format}' not found in _squados/core/best-practices/. Skipping format injection." Continue without format.
