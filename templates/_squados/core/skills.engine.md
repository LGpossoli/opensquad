...
- **Skill format reference**: `skills/squados-skill-creator/references/skill-format.md`
...
For the full SKILL.md specification, see `skills/squados-skill-creator/references/skill-format.md`.
...
2. **Fetch SKILL.md from GitHub**:
   ...
   ```
   - If fetch fails (404 or network error) → **ERROR**: "Skill '<name>' not found in the SquadOS catalog."
...
1. Check if the `squados-skill-creator` skill is installed:
   - Look for `skills/squados-skill-creator/SKILL.md`

2. **If installed** → read `skills/squados-skill-creator/SKILL.md` and follow its
...
3. **If not installed** → inform the user:
   ...
   `/squados skills` → Install → `squados-skill-creator`"
...
3. **For each non-native skill**:
   ...
   c. **Verify MCP configuration** (if `type: mcp` or `type: hybrid`):
      ...
      - If missing → **ERROR**: "Skill '{skill}' is installed but its MCP server is not configured. Run `/squados skills` → Install to reconfigure."
