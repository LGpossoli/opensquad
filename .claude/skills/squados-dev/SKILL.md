...
|------|-----------|----------|
| SHARED | Applies to all IDEs equally | `_squados/core/`, `templates/` (excluding `ide-templates/`) |
...
| Antigravity | `templates/ide-templates/antigravity/` | `.antigravity/rules.md`, `.agent/workflows/squados.md` |
...
- ❌ Wrong: Edit `_squados/core/runner.pipeline.md` adding `if antigravity: run sequentially`
...
- ✅ Fine: Edit `_squados/core/architect.agent.yaml` because the change benefits ALL IDEs equally.
...
#### Check J: IDE contamination in shared files (any `_squados/core/**` or `templates/**` changed, excluding `templates/ide-templates/**`)

Scan shared files for IDE-specific conditional logic that should live in `templates/ide-templates/{ide}/` instead.

**Files to scan:**
- All files in `_squados/core/`
